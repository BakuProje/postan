    ...
    <div class="bg-white rounded-2xl border border-neutral-200/60 p-8">
        <form method="POST" action="{{ route('admin.users.update', $user->id) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="flex flex-col items-center justify-center pb-6 border-b border-neutral-100">
                    ...
                    <label for="profile_picture" class="cursor-pointer block relative">
                        ...
                        <div class="h-24 w-24 rounded-full bg-gradient-to-tr from-neutral-50 to-neutral-100 border-2 border-neutral-200 flex items-center justify-center overflow-hidden transition duration-350 group-hover:scale-102 group-hover:border-sky-400 relative">
                            @if($user->profile_picture)
                                <img id="avatar-preview" src="{{ asset($user->profile_picture) }}" alt="Preview" class="h-full w-full object-cover">
                                <svg id="avatar-placeholder" class="hidden h-10 w-10 text-neutral-300 transition group-hover:text-sky-500" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                                </svg>
                            @else
                                <img id="avatar-preview" src="#" alt="Preview" class="hidden h-full w-full object-cover">
                                <div id="avatar-placeholder" class="h-full w-full bg-sky-50 text-sky-600 border border-sky-100 flex items-center justify-center font-bold text-sm">
                ...
                <span class="text-[10px] text-neutral-400 mt-3 font-medium">Ketuk untuk ubah foto.</span>
            ...
            <div class="grid gap-6 sm:grid-cols-2">
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <input id="name" name="name" type="text" value="{{ old('name', $user->name) }}" required placeholder="Nama lengkap karyawan" class="block w-full rounded-xl border border-neutral-200 bg-neutral-50/30 pl-10 pr-4 py-3 text-xs outline-none transition duration-200 placeholder:text-neutral-400/80 focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-100/50 @error('name') border-rose-500 @enderror">
                    </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <select id="shift" name="shift" class="block w-full rounded-xl border border-neutral-200 bg-neutral-50/30 pl-10 pr-4 py-3 text-xs outline-none appearance-none transition duration-200 focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-100/50">
                            <option value="" {{ old('shift', $user->shift) === '' ? 'selected' : '' }}>Tanpa Shift (Default/Kantor)</option>
                            <option value="Pagi" {{ old('shift', $user->shift) === 'Pagi' ? 'selected' : '' }}>Shift Pagi</option>
                            <option value="Siang" {{ old('shift', $user->shift) === 'Siang' ? 'selected' : '' }}>Shift Siang</option>
                            <option value="Malam" {{ old('shift', $user->shift) === 'Malam' ? 'selected' : '' }}>Shift Malam</option>
                        </select>
                        <span class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none text-neutral-400">
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
