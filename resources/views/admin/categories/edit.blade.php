...
<div class="max-w-md mx-auto">
    <div class="bg-white rounded-2xl border border-neutral-200/60 p-6 space-y-6">
        <div>
            <h2 class="text-lg font-black text-neutral-900 tracking-tight">Ubah Kategori</h2>
            <p class="text-xs text-neutral-500 mt-1">Ubah nama kategori produk di sistem.</p>
        </div>
        <form method="POST" action="{{ route('admin.categories.update', $category->id) }}" class="space-y-4">
            ...
            <div class="space-y-2">
                ...
                <input id="name" name="name" type="text" value="{{ old('name', $category->name) }}" required placeholder="Contoh: Makanan, Minuman" class="block w-full rounded-xl border border-neutral-200 bg-neutral-50/30 px-4 py-3 text-xs outline-none transition duration-200 placeholder:text-neutral-400/80 focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-100/50">
                @error('name')
