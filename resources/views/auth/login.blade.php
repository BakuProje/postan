...
<body class="min-h-screen bg-neutral-50/50 font-sans text-neutral-800 antialiased selection:bg-sky-500 selection:text-white">
    <main class="grid min-h-screen lg:grid-cols-2">
        <section class="relative hidden bg-gradient-to-br from-sky-100 via-sky-50 to-sky-200/30 p-12 text-neutral-800 lg:flex lg:flex-col lg:justify-between overflow-hidden border-r border-neutral-200/50">
            ...
            <a href="{{ url('/') }}" class="relative z-10 flex items-center gap-2.5 text-base font-bold tracking-wider text-neutral-900 transition-transform duration-250 hover:scale-[1.01]">
            ...
            <div class="relative z-10 max-w-md">
                <p class="mb-5 text-[10px] font-bold tracking-widest text-sky-900 border border-sky-250 bg-sky-100/50 w-fit px-2.5 py-1 rounded-md uppercase">Kelola Toko Lebih Praktis</p>
                <h1 class="text-3xl font-extrabold leading-tight tracking-tight text-neutral-900 font-sans">Semua transaksi toko Anda, dalam satu tempat.</h1>
                <p class="mt-4 text-sm leading-relaxed text-neutral-500">Pantau penjualan, kelola stok produk, dan pantau performa kasir harian Anda dengan lebih praktis.</p>
            </div>
        ...
        <section class="relative flex items-center justify-center px-6 py-12 sm:px-10 overflow-hidden bg-white lg:bg-transparent">
            ...
            <div class="w-full max-w-md">
                ...
                <div class="mb-8 text-center">
                    <span class="text-[10px] font-bold uppercase tracking-wider text-sky-800 border border-sky-200 bg-sky-50/30 px-2.5 py-1 rounded">Selamat Datang Kembali</span>
                    <h2 class="mt-4 text-2xl font-extrabold tracking-tight text-neutral-900">Masuk ke akun Anda</h2>
                    <p class="mt-2 text-xs text-neutral-500">Masukkan email dan kata sandi untuk melanjutkan ke dasbor.</p>
                </div>

                @if (session('status'))
                    <div class="mb-5 rounded-lg border border-emerald-250 bg-emerald-50 px-4 py-3 text-xs text-emerald-700">{{ session('status') }}</div>
                @endif
                ...
                <form method="POST" action="{{ route('login.store') }}" class="space-y-5">
                    ...
                    <label class="flex cursor-pointer items-center gap-2.5 text-xs text-neutral-500">
                        <input type="checkbox" name="remember" class="size-4 rounded border-neutral-200 text-sky-600 focus:ring-blue-500">
                        Ingat saya di perangkat ini
                    </label>
                    
                    <button type="submit" class="w-full rounded-lg bg-sky-500 px-4 py-3 text-xs font-semibold text-white transition-all duration-200 hover:bg-sky-600 shadow-sm">Masuk</button>
                </form>
            </div>
        </section>
    </main>
